                        ...
                        QPainter *painter, const QWidget *widget) const
{
    ...
    case PE_IndicatorHeaderArrow:
        if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader *>(option)) {
            ...
            if (!arrow.isNull()) {
                ...
                arrow.setColor(1, header->palette.color(QPalette::WindowText).rgba());
    ...
    case PE_IndicatorToolBarSeparator:
        {
            ...
            if (option->state & State_Horizontal) {
                ...
                painter->setPen(QPen(option->palette.color(QPalette::Window).darker(110)));
                painter->drawLine(rect.bottomLeft().x() + offset,
                            rect.bottomLeft().y() - margin,
                            rect.topLeft().x() + offset,
                            rect.topLeft().y() + margin);
                painter->setPen(QPen(option->palette.color(QPalette::Window).lighter(110)));
    ...
    case PE_FrameMenu:
        painter->save();
        {
            ...
            painter->drawRect(option->rect.adjusted(0, 0, -1, -1));
            QColor frameLight = option->palette.color(QPalette::Window).lighter(160);
            QColor frameShadow = option->palette.color(QPalette::Window).darker(110);
    ...
    case PE_FrameDockWidget:
        ...
        painter->save();
        {
            QColor softshadow = option->palette.color(QPalette::Window).darker(120);
            ...
            painter->setPen(QPen(option->palette.color(QPalette::Window).darker(120), 0));
    ...
    case PE_FrameWindow:
        painter->save();
        {
            ...
            if (widget && widget->inherits("QMdiSubWindow")) {
				...
				QColor menuBackground = option->palette.color(QPalette::Window).lighter(104);
                                   ...
                                   const QWidget *widget) const
{
    ...
    darkOutline.setHsv(button.hue(),
                ...
                qMin(255, (int)(button.value()*0.6)));
    QRect rect = option->rect;
    QColor shadow = mergedColors(option->palette.color(QPalette::Window).darker(120),
                                 dark.lighter(130), 60);
    ...
    case CE_ToolBar:
        ...
        if (const QStyleOptionToolBar *toolbar = qstyleoption_cast<const QStyleOptionToolBar *>(option)) {
            ...
            case Qt::TopToolBarArea:
                ...
                if (toolbar->direction == Qt::RightToLeft) {
            ...
            QColor light = option->palette.color(QPalette::Window).lighter(110);
                                         ...
                                         QPainter *painter, const QWidget *widget) const
{
    ...
    if (widget) {
        ...
        alphaCornerColor = mergedColors(option->palette.color(QPalette::Window), darkOutline);
    }
    ...
    case CC_TitleBar:
        ...
        if (const QStyleOptionTitleBar *titleBar = qstyleoption_cast<const QStyleOptionTitleBar *>(option)) {
            ...
            QColor titleBarHighlight(active ? highlight.lighter(120): palette.color(QPalette::Window).lighter(120));
                                        ...
                                        const QSize &size, const QWidget *widget) const
{
    ...
    case CT_MenuItem:
        if (const QStyleOptionMenuItem *menuItem = qstyleoption_cast<const QStyleOptionMenuItem *>(option)) {
			...
			width+=fm.boundingRect(itemText).width();
			...
			if (shotcutList.count() > 0) {
				...
				width+=fm.boundingRect(shotcutList.last()).width() + 4;
			}
                               ...
                               QStyleHintReturn *returnData) const
{
    ...
    case SH_Table_GridLineColor:
        if (option) {
            ret = option->palette.color(QPalette::Window).darker(120).rgb();
        }
        break;
